...
-- 11.	Find the total revenue from active subscriptions.==>vinayak 11
...
-- 12.	Count how many users joined in each month. ==>vinayak 12
SELECT DATE_FORMAT(max(Last_Login), '%Y-%m') AS Month, COUNT(User_ID) AS User_Count FROM netflix_users
GROUP BY Last_Login
ORDER BY Month desc;

-- 13.	List all users who have a watch time greater than the average watch time ==> nived.13
SELECT * 
FROM netflix_users 
WHERE Watch_Time_Hours > (SELECT AVG(Watch_Time_Hours) FROM netflix_users);

-- 14.	Find the user with the longest watch time in each country. ==> Nived.14
